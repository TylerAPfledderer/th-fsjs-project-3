...
const validStyle = (containerEl, bool, errorText = "") => {
  ...
  const classExist =
    containerEl.classList.contains("valid") ||
    containerEl.classList.contains("not-valid");
  ...
  !classExist
    ? containerEl.classList.add(selectorName)
    : containerEl.classList.replace(prevSelector, selectorName);
    ...
    function changeActivityDisable(activities, bool) {
      activities.forEach((similarActivity) => {
        if (
          ...
          similarActivity.dataset.dayAndTime === dayAndTime
        ) {
  ...
  const validInputTargets = [
    {
      ...
      errorHint(value, regex) {
        return errorDisplayText(
          ...
          "Your name can not begin or only contain spaces."
        );
      },
    },
    {
      ...
      errorHint(value, regex) {
        return errorDisplayText(
          ...
          "Your email address needs to have the correct format."
        );
      },
    },
    {
    ...
    },
    {
      ...
      errorHint(value, regex) {
        return errorDisplayText(
          value,
          regex,
          "Please Enter the Zip Code for the Credit Card.",
    ...
    },
    {
      ...
      errorHint(value, regex) {
        return errorDisplayText(
          value,
          regex,
          "Please Enter The Credit Card's Security Code",
...
form.addEventListener("submit", (event) => {
  ...
  inputGroup.forEach((input) => {
    if (input === ccNumber || input === ccZip || input === ccCVV) {
      if (
  ...
  if (
    Array.from(activitiesGroup).every((checkBox) => checkBox.checked === false)
  ) {
